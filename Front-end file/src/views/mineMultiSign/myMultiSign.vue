<template>
  <div class="myMultiSign">
    ...
    <div class="list">
      <div class="item"  v-for="(item,index) in list" :key="index" @click="goMulPanel(item)">
        <div class="title">
          MultiSign  {{index+1}}
        </div>
        <div class="address">
...
export default {
  ...
  methods: {
    goMulPanel(address){
      ...
      this.$store.commit("multiSign/SET_MULTIADDRESS",address)
      this.$router.push({name:'multiSignPanel'})
    },
    getData(){
      this.list = []
